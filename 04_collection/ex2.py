...
print(a, b, c)
# a, b, c = zip(subjects, scores, strict=True) # strict 쓰면 크기 다를 때 에러
# print(a, b, c)

for subject, score in zip(subjects, scores):
    print(f"{subject}: {score}점")

print(list(map(list, zip(subjects, scores))))
print(list(zip(subjects, scores))) # list(x) : x를 순회해서 리스트에 넣음
print([zip(subjects, scores)]) # [x] : x 자체를 원소 하나로 해서 넣음
...
result = [i * j for j in y for i in x]
print(result)
